...
//#define File FILE *
    psave_to_file(fho,tb,cooc_vec,&g,opts);
    timer.stop();
...
//    MPI::COMM_WORLD.Barrier();
    time_str=Crono::formatted_min_max_span(timer);
    ROOT_MSG cout<<"Table successfully created and written in "
           <<time_str
            <<endl;
...
//     opt->addUsage( " -r  --match-raw-text\t\tOptionally write only matching source phrases to the provided test set (default: '')" );
	...
	opt->addUsage( " -x  --xi-func\t\t{kn|hyp|hyp1|log|log1|exp|exp1} the discounting function (xi). (default: kn)" );    
    ...
    opt->addUsage( " -j  --join-lexical\t\t{average|noisy-or|a|nor} Method used to combine lexical scores. (default: average)" );
...
//        ROOT_MSG cout<<"No output file name was specified!\nThe output is redirected to 'stderr' and the phrase table will be written to 'stdout'\n";
    ...
    if( output_fn=="" && (opt->getFlag( "compress" ) || opt->getFlag( 'c' ))) {
        ROOT_MSG cout<<"Output table will be gzipped\n";
    }
