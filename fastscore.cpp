...
void do_sort(element_vector* in_container,element_vector* out_container,unsigned long mem)
{
    DemsortSettings s;
//    s.mwsel_variant=DemsortSettings::MWSEL_VARMAN;
    s.memory_per_pe = mem;
    s.verbose = 0;
    in_container->flush();
    distributed_external_sort(*in_container, *out_container,
                              in_container->begin(), in_container->end(), CmpKey()
//                         MinMaxLess<value_type>()
                              , s);
}
void  uniqify(local_vector * res_vec,element_vector * orig,element_vector::const_iterator * merger, count_t * ut_ptr, const Lexical_aggregator * agg,
...
//   //
    ...
    while(local_itr!=tmp_vec.end())
    {
        ...
        local_itr->ut=acc->accumulator(sum_t,ut,0);
//            local_itr->invn1=n[0];
//            local_itr->invn2=n[1];
...
void accumulate_mass(local_vector * tmp_vec,element_vector * orig,element_vector::const_iterator * merger,
                      const Discounter * d)
{
...
void score_one_source(local_vector * tmp_vec,element_vector * orig,element_vector::const_iterator * merger,
                      count_t * us_ptr,
// 					  const Discounter * d,
					  const Smoothing_att_calculator *acc, unsigned long * text_bytes)//,unsigned long * klen,unsigned long * salen,unsigned long * talen)
{
//    std::vector<pair_t> & tmp_vec=*cooc_vec;
    unsigned long size0=tmp_vec->size();
    //Crono timer;
...
//         local_itr->for_score=d->discount(local_itr->cooc);
    ...
    for(local_itr=tmp_vec->begin()+size0; local_itr!=tmp_vec->end(); ++local_itr)
    {
        ...
        local_itr->us=acc->accumulator(sum_s,us,0);
...
/*
element_vector::const_iterator get_next_phrase_index(element_vector * cooc_vec,element_vector::const_iterator * start)
{
    cooc_type::key_t new_src,cur_src;

    if(*start==cooc_vec->end())
        return *start;

//    while(start !=cooc_vec->end()){

    strcpy(new_src,(*start)->_key);
    //strcpy(cur_src,(*merger)->key());
    break_pair(new_src);//,STR_SEP)='\0';
    strcpy(cur_src,new_src);
    //memcpy(cur_src,new_src,strlen((*merger)->key())+1);
...
*/
void copy_to_global_and_count(local_vector * myvec, element_vector * cooc, count_map_t * counts, omp_lock_t * global_lock)
{
    if(myvec->empty())
        return;
...
unsigned long score(element_vector ** cooc_vec_ptr,global_t* g_scores,const Options * opts)
{
...
//    cooc_vec->print_sizes();
	...
	ROOT_MSG cout<<"Scoring using 'cooc'"<<endl;
...
//   phase1span=timer.span();
    ROOT_MSG cout<<"\n\t"<<"Done sorting\nSorting took "<<timer.formatted_span()<<endl;
    //print_cooc_vec(cooc_vec);
    //cout<<*cooc_vec<<endl;
    ROOT_MSG cout<<"\n-------------------------\n"<<"Uniqifying phrase pairs\n-------------------------"<<endl;
...
//
        ...
        while(g_merger!=cooc_vec->end())
        {
            try
            {
                ...
                uniqify(&myvec,cooc_vec,&g_merger, &u, &agg, &acc);
...
//         }
    ...
    ROOT_MSG cout<<endl<<"Done uniqifying in: "<<timer.formatted_span()<<endl;
...
//     ROOT_MSG cout<<endl<<"Process " << MPI::COMM_WORLD.Get_rank() << ": Number of non-identical pairs: "<<gs.un<<endl;
    ...
    ROOT_MSG cout<<endl<<"Tuning XI function parameter(s) ..."<<endl<<"XI function: "<<opts->xi_func<<endl;
...
//         }
    ...
    ROOT_MSG cout<<endl<<"Done accumulating in: "<<timer.formatted_span()<<endl;
//    phase1span+=timer.span();
//    timer.reset();
//    timer.start();
//    res_vec->print_sizes();
...
//    phase2span=timer.span();
    ROOT_MSG cout<<endl<<"Done sorting partial sets\nSorting took "<<timer.formatted_span()<<endl;
...
//     unsigned long kbytes=0,sabytes=0,tabytes=0;
    myvec.clear();
#pragma omp parallel private(myvec) shared(lock,g_merger,cooc_vec,res_vec,acc),reduction(+:u), reduction(+:bytes)
...
//                      nowait
        ...
        while(g_merger!=cooc_vec->end())
        {
            try
            {
                score_one_source(&myvec,cooc_vec,&g_merger,&u,
// 								 &d,
								 &acc,&bytes); //,&kbytes,&sabytes,&tabytes);//score_one_source(&lock,&lq,res_vec,g_merger);
            }
            catch(VectorOverloaded & o)
            {
...
//     cout<<endl<<MPI::COMM_WORLD.Get_rank()<<": Done merging in: "<<timer.formatted_span()<<endl;
