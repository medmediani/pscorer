...
public:
	...
	Unseen_estimator(string estimator=""):estimate(Unseen_estimator::none)
	{
        if (estimator=="chao" || estimator=="c"){
            ROOT_MSG cout<<"Estimating the number of unseen events using 'Chao estimator'"<<endl;
            estimate=Unseen_estimator::chao;
        }else if (estimator=="bias-corrected-chao" || estimator=="b"){
            ROOT_MSG cout<<"Estimating the number of unseen events using 'Bias-corrected-Chao estimator'"<<endl;
			estimate=Unseen_estimator::bcorrected_chao;
		}else if (estimator=="nonparametric-lindley" || estimator=="l"){
            ROOT_MSG cout<<"Estimating the number of unseen events using 'Mixed Poisson-Lindley estimator'"<<endl;
			estimate=Unseen_estimator::np_lindley;
		}else if (estimator=="geometric" || estimator=="g"){
            ROOT_MSG cout<<"Estimating the number of unseen events using 'Mixed Exponential estimator'"<<endl;
			estimate=Unseen_estimator::mix_exp;
		}else if (estimator=="good-turing" || estimator=="gt"){
            ROOT_MSG cout<<"Estimating the number of unseen events using 'Good-Turing estimator'"<<endl;
			estimate=Unseen_estimator::gt;
		}else{
            ROOT_MSG cout<<"Estimating the number of unseen events using 'Leaving-One-Out'"<<endl;
        }
